...
export function render(system, ctx) {
    let y = 5;
    ...
    function list(gruppe, text, last) {
        ...
        y += 23;
        if (!(gruppe instanceof System || gruppe instanceof Gruppe))
            return;
        for (let i = 0; i < gruppe.elemente.length; i++) {
            list(gruppe.elemente[i], text + (i == gruppe.elemente.length - 1 ? "  " : "\u2502 "), i == gruppe.elemente.length - 1);
        }
    }
    list(system, "", true);
}
